...
impl PipeRpcConnection {
    ...
    ///
    /// Some servers (e.g. Samba) require this byte-stream exchange for DCE/RPC bind PDUs instead of
    /// `FSCTL_PIPE_TRANSCEIVE`.
    #[maybe_async]
    pub async fn dcerpc_pipe_exchange(
        pipe: &mut Pipe,
        call_id: u32,
        to_send: DcRpcCoPktRequestContent,
    ) -> crate::Result<DceRpcCoResponsePkt> {
        Self::rpc_rw(pipe, call_id, to_send).await
    }
    ...
    async fn rpc_rw(
        ...
        let buf: &[u8] = content.buffer.as_ref();
        if buf.len() >= 28 && buf[2] == 3 {
            let status = u32::from_le_bytes(buf[24..28].try_into().map_err(|_| {
                crate::Error::InvalidMessage("DCE/RPC fault PDU too short".to_string())
            })?);
            return Err(crate::Error::InvalidMessage(format!(
                "DCE/RPC fault status={status:#010x}"
            )));
        }
        let response = DceRpcCoResponsePkt::try_from(buf)?;
    ...
    /// Bound pipe after a custom DCE/RPC bind (e.g. classic NDR for Samba).
    pub fn new_bound(
        pipe: Pipe,
        next_call_id: u32,
        context_id: u16,
        ...
        Self {
            ...
            _server_max_recv_frag: server_max_recv_frag,
        }
    }
}
...
impl BoundRpcConnection for PipeRpcConnection {
    ...
    async fn send_receive_raw(
        ...
        let buf: &[u8] = res.as_ref();
        if buf.len() >= 28 && buf[2] == 3 {
            let status = u32::from_le_bytes(buf[24..28].try_into().map_err(|_| {
                SmbRpcError::SendReceiveError("DCE/RPC fault PDU too short".to_string())
            })?);
            return Err(SmbRpcError::SendReceiveError(format!(
                "DCE/RPC fault status={status:#010x}"
            )));
        }
        let rpc_reply = DceRpcCoResponsePkt::try_from(buf)
