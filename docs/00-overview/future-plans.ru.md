...
| FP-7e | (done) `POST /library/scan?root=` + UI Repair folder на альбоме                             |
| FP-7f | (done) Rebuild index, invalidate Library после download, Download/Re-download lock в Favorites |
...
| FP-9b | (done) Re-enqueue + `visited` + level-wise `read_dir`; stress-тесты |
| FP-9c | ✅ Env + cap workers; [library-scan.ru.md](../02-backend/library-scan.ru.md) |
| FP-9d | (done) Skip unchanged (`mtime` + `file_size`); миграция `tracks.file_size` |
