...
pub(super) fn create_cue_jobs(_: &TableState) -> welds::errors::Result<MigrationStep> {
    let migration = create_table("cue_jobs")
        .id(|c| c("id", Type::IntBig))
        .column(|c| {
            c("album_id", Type::IntBig).create_foreign_key("albums", "id", OnDelete::Cascade)
        })
        .column(|c| c("status", Type::String))
        .column(|c| c("tracks_total", Type::IntBig))
        .column(|c| c("tracks_done", Type::IntBig))
    ...
    Ok(MigrationStep::new("025_create_cue_jobs", migration))
}
